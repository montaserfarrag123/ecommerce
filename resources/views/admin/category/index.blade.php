...
@section('content')
    <div class="card">
        <div class="card-header">
            <a href="{{ url('/createCat') }}" class="btn btn-primary">Add Category</a>
            <h3>Category Page</h3>
            {{-- @if (session('status'))
                ...
                <tbody>
                    ...
                    @foreach ($categories as $item)
                        <tr>
                            ...
                            <td>
                                @if ($item->image)
                                    <img src="{{ asset('assets/category/' . $item->image) }}" width=50; height=50;
                                        srcset="">
                            </td>
                        @else
                            {{ 'لم يتم ارفاق الصورة ' }}
                        ...
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="{{url('deleteCat/'.$item->id)}}" method="post">
                                        @csrf
                                        @method('delete')
